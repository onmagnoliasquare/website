...
let lastId = $derived.by<string | null>(() =>
  articles.length > 0 ? articles[articles.length - 1]._id : null
)
let lastDate = $derived.by<string | null>(() =>
  articles.length > 0 ? articles[articles.length - 1].date : null
)
...
<div class="m-2 p-2">
  {#key category._id}
    ...
    {#if articles.length == 0}
      <P class="mt-4 w-full pl-3 text-left text-neutral-500 italic">Nothing here yet...</P>
    {:else}
      <MoreArticles
        lastId={lastId}
        lastDate={lastDate}
