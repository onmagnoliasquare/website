...
test('News -> Opinion -> People -> Culture -> Multimedia -> Travel categories', async ({
  page,
}) => {
  await page.goto('/category/news')
  await page.locator('#mainNav').getByRole('link', { name: 'Opinion' }).click()
  ...
  await page.locator('#mainNav').getByRole('link', { name: 'Travel' }).click()
  await expect.soft(page.getByRole('heading', { name: 'Travel', exact: true })).toBeVisible()
})
